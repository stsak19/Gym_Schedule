/* Service worker του γυμναστηρίου
   Δύο δουλειές:
...
/* Το όνομα του γυμναστηρίου το ορίζει ο διαχειριστής. Ο service
   worker δεν μπορεί να ρωτήσει τη βάση όποτε θέλει, οπότε του το
   στέλνει η σελίδα σε κάθε άνοιγμα και το κρατάμε εδώ. */
const BRAND_KEY = "./__brand";

async function readBrand() {
  try {
    const cache = await caches.open(RUNTIME);
    const hit = await cache.match(BRAND_KEY);
    if (!hit) return null;
...
async function writeBrand(name) {
  try {
    const cache = await caches.open(RUNTIME);
    await cache.put(BRAND_KEY, new Response(JSON.stringify({ name: name }), {
...
self.addEventListener("message", (event) => {
  const d = event.data;
  if (d && d.type === "brand" && d.name) event.waitUntil(writeBrand(d.name));
});
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil((async () => {
    const brand = (await readBrand()) || "Γυμναστήριο";
